...
Here is a sample code. After run this code it will show an SVG image whose source code is saved on Ethereum:
...
const CONTRACT_ADDRESS = '0xefb21d94Bbd3271601c64AA7829fFc69788BDC98'
const TOKEN_ID = "0x0200000000000000000000000000000003"
const abi = [
  "function getTokenImageSvg(uint tokenId) view returns (string)"
];
...
function CodeDemo() {
    ...
    useEffect(()=>{
      ...
      contract.getTokenImageSvg(TOKEN_ID).then( svg_code => {
        refItem.current.innerHTML = svg_code;
      }).catch(err =>{
